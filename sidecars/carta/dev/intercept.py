...
def intercept(
    ...
    echo_deploy: str = typer.Option("carta-echo", "--echo-deploy", help="Echo Deployment name (default: carta-echo)"),
):
    """
    ...
    # 2) kubectl apply
    print(f"→ Applying interceptor into ns={namespace}, session={session_id} ...", flush=True)
    try:
    ...
    # 3) wait a bit
    print(f"→ Waiting {wait_seconds:.0f}s for resources to become ready ...", flush=True)
    time.sleep(wait_seconds)

    # 4) stream logs from echo deployment
    dep_name = get_deploy_name(namespace, fallback=echo_deploy)
    print(f"→ Tailing logs from Deployment/{dep_name} in ns={namespace} (Ctrl-C to stop) ...", flush=True)
